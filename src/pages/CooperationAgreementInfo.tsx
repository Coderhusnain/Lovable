...
import { cooperationAgreementInfo } from "@/data/cooperationAgreementInfo";

const CooperationAgreementInfo: React.FC = () => {
  const navigate = useNavigate();
  const info = cooperationAgreementInfo;
    ...
    <Layout>
      <div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-cyan-50 py-12">
        <div className="max-w-6xl mx-auto px-4">
            ...
            {info.otherNames && info.otherNames.length > 0 && (
              <div className="flex flex-wrap gap-2 mb-6">
                {info.otherNames.map((name) => (
                  <span key={name} className="px-3 py-1 bg-cyan-100 text-cyan-700 text-sm rounded-full font-medium">
                    {name}
                  </span>
                ))}
              </div>
          ...
          <div className="mb-12 bg-gradient-to-r from-slate-50 to-cyan-50 border border-cyan-200 rounded-lg p-8">
          ...
          {info.howItWorks && (
            <div className="mb-12 bg-white border border-cyan-200 rounded-lg p-8">
              <h2 className="text-2xl font-bold text-gray-900 mb-4">How Does a Cooperation Agreement Work?</h2>
            ...
            <div className="mb-12">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {info.keyBenefits.map((benefit, idx) => (
                  <div key={idx} className="flex gap-3 p-4 bg-cyan-50 rounded-lg border border-cyan-200">
                    <CheckCircle className="w-5 h-5 text-cyan-600 mt-0.5 flex-shrink-0" />
            ...
            <div className="mb-12">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {info.idealFor.map((person, idx) => (
                  <div key={idx} className="flex gap-3 p-4 bg-white rounded-lg border border-cyan-200">
                    <CheckCircle className="w-5 h-5 text-cyan-600 mt-0.5 flex-shrink-0" />
            ...
            <div className="mb-12">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {info.whatIncludes.map((item, idx) => (
                  <div key={idx} className="p-6 bg-white rounded-lg border border-cyan-200">
            ...
            <div className="mb-12">
              ...
              <div className="space-y-4">
                {info.faqItems.map((faq, idx) => (
                  <details key={idx} className="p-4 bg-white rounded-lg border border-cyan-200 cursor-pointer hover:border-cyan-300 transition">
          ...
          {info.highlights && info.highlights.length > 0 && (
            <div className="mb-12">
              <h2 className="text-2xl font-bold text-gray-900 mb-6">Why Choose Our Cooperation Agreement?</h2>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {info.highlights.map((highlight, idx) => (
                  <div key={idx} className="flex gap-3 p-4 bg-gradient-to-r from-cyan-50 to-cyan-100 rounded-lg border border-cyan-200">
                    <CheckCircle className="w-5 h-5 text-cyan-600 mt-0.5 flex-shrink-0" />
          ...
          <div className="text-center py-12">
            <h2 className="text-3xl font-bold text-gray-900 mb-4">Ready to Create Your Cooperation Agreement?</h2>
            <p className="text-gray-700 mb-8 text-lg">Get started with our professionally designed template today.</p>
            <Button size="lg" onClick={() => navigate("/cooperation-agreement-form")} className="bg-cyan-600 hover:bg-cyan-700 text-lg px-8 text-white">
...
export default CooperationAgreementInfo;