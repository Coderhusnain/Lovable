...
const JointBidInfo: React.FC = () => {
  const navigate = useNavigate();
  const doc = getDocumentContent("Joint Bid Agreement");

  return (
    <Layout>
      <div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-sky-50 py-12">
        <div className="max-w-6xl mx-auto px-4">
            ...
            {doc.otherNames && doc.otherNames.length > 0 && (
              <div className="flex flex-wrap gap-2 mb-6">
                {doc.otherNames.map((name) => (
                  <span key={name} className="px-3 py-1 bg-sky-100 text-sky-700 text-sm rounded-full font-medium">
                    {name}
                  </span>
                ))}
              </div>
            ...
            <div className="mb-12">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {doc.whatYouNeed.map((item, idx) => (
                  <div key={idx} className="flex gap-3 p-4 bg-sky-50 rounded-lg border border-sky-200">
                    <CheckCircle className="w-5 h-5 text-sky-600 mt-0.5 flex-shrink-0" />
            ...
            <div className="mb-12">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {doc.keyProtections.map((protection, idx) => (
                  <div key={idx} className="flex gap-3 p-4 bg-gradient-to-br from-sky-50 to-indigo-50 rounded-lg border border-sky-200">
                    <CheckCircle className="w-5 h-5 text-sky-600 mt-0.5 flex-shrink-0" />
            ...
            <div className="mb-12">
              ...
              <div className="space-y-4">
                {doc.faqs.map((faq, idx) => (
                  <div key={idx} className="bg-white border border-sky-200 rounded-lg p-6 hover:shadow-md transition-shadow">
                    <h3 className="font-bold text-gray-900 mb-3 text-sky-700">{faq.q}</h3>
          ...
          <div className="text-center space-y-4">
            <Button
              onClick={() => navigate("/documents/joint-bid-agreement")}
              size="lg"
              className="bg-gradient-to-r from-sky-600 to-indigo-600 hover:from-sky-700 hover:to-indigo-700 text-white px-8 py-3"
            >
              Create Joint Bid Agreement
            </Button>
