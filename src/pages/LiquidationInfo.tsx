...
const LiquidationInfo: React.FC = () => {
  ...
  const doc = getDocumentContent("Liquidation Agreement");

  return (
    <Layout>
      <div className="min-h-screen bg-gradient-to-br from-stone-50 via-white to-amber-50 py-12">
        <div className="max-w-6xl mx-auto px-4">
          <div className="mb-12">
            <div className="flex items-center gap-4 mb-6">
              <div className="p-3 bg-stone-100 rounded-lg">
                <Archive className="w-8 h-8 text-stone-700" />
              </div>
            ...
            {doc.otherNames && doc.otherNames.length > 0 && (
              <div className="flex flex-wrap gap-2 mb-6">
                {doc.otherNames.map((name) => (
                  <span key={name} className="px-3 py-1 bg-stone-100 text-stone-800 text-sm rounded-full font-medium">
                    {name}
                  </span>
                ))}
              </div>
              ...
              <ul className="space-y-3">
                {doc.whenToUse.map((use, idx) => (
                  <li key={idx} className="flex gap-3 items-start">
                    <CheckCircle className="w-5 h-5 text-stone-700 mt-0.5 flex-shrink-0" />
            ...
            <div className="mb-12">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {doc.whatYouNeed.map((item, idx) => (
                  <div key={idx} className="flex gap-3 p-4 bg-stone-50 rounded-lg border border-stone-200">
                    <CheckCircle className="w-5 h-5 text-stone-700 mt-0.5 flex-shrink-0" />
            ...
            <div className="mb-12">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {doc.keyProtections.map((protection, idx) => (
                  <div key={idx} className="flex gap-3 p-4 bg-gradient-to-br from-stone-50 to-amber-50 rounded-lg border border-stone-200">
                    <CheckCircle className="w-5 h-5 text-stone-700 mt-0.5 flex-shrink-0" />
            ...
            <div className="mb-12">
              ...
              <div className="space-y-4">
                {doc.faqs.map((faq, idx) => (
                  <div key={idx} className="bg-white border border-stone-200 rounded-lg p-6 hover:shadow-md transition-shadow">
          ...
          <div className="text-center space-y-4">
            <Button
              onClick={() => navigate("/documents/liquidation-agreement")}
              size="lg"
              className="bg-stone-800 hover:bg-stone-900 text-white px-8 py-3"
            >
              Create Liquidation Agreement
            </Button>
