...
const generatePDF = (values: Record<string, string>) => {
  ...
  doc.text("1. PURPOSE AND SCOPE", margin, y); y += lh + 1;
  para(values.description || "N/A", false, lh);

  // ── 2. TERMS AND CONDITIONS ──────────────────────────────────────────────────
  doc.setFont("helvetica", "bold"); doc.setFontSize(fs); doc.setTextColor(0, 0, 0);
  doc.text("2. TERMS AND CONDITIONS", margin, y); y += lh + 1;
  para(
  ...
  if (values.paymentAmount || values.paymentSchedule) {
    doc.setFont("helvetica", "bold"); doc.setFontSize(fs); doc.setTextColor(0, 0, 0);
    doc.text("3. FINANCIAL TERMS", margin, y); y += lh + 1;
    para(`Payment Amount: ${values.paymentAmount || "N/A"}.  Schedule: ${values.paymentSchedule || "N/A"}.`, false, lh);
  }

  // ── 4. ADDITIONAL TERMS ──────────────────────────────────────────────────────
  if (values.additionalTerms) {
    doc.setFont("helvetica", "bold"); doc.setFontSize(fs); doc.setTextColor(0, 0, 0);
    doc.text("4. ADDITIONAL TERMS", margin, y); y += lh + 1;
    const addLines = doc.splitTextToSize(values.additionalTerms, textWidth).slice(0, 2);
    doc.setFont("helvetica", "normal");
    doc.text(addLines, margin, y);
    y += addLines.length * lh + lh;
  }

  para("IN WITNESS WHEREOF, the parties have executed this Agreement as of the date first written above.", false, lh + 2);
  ...
  if (sig1) { doc.setDrawColor(0,0,0); doc.line(margin, y + 1, margin + doc.getTextWidth(sig1), y + 1); }
  y += lh + 1;
  ...
  if (sig2) { doc.setDrawColor(0,0,0); doc.line(margin, y + 1, margin + doc.getTextWidth(sig2), y + 1); }
  y += lh + 1;
  doc.setFont("helvetica", "normal"); doc.setFontSize(fs);
  ...
  if (values.witnessName) {
    ...
    doc.setDrawColor(0,0,0); doc.line(margin, y + 1, margin + doc.getTextWidth(values.witnessName), y + 1);
    y += lh + 1;
    doc.setFont("helvetica", "normal");
  ...
  doc.text(`Generated by Legalgram  •  ${new Date().toLocaleDateString()}`, pageWidth / 2, 290, { align: "center" });
