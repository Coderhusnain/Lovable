...
const generatePDF = (values: Record<string, string>) => {
  ...
  doc.setFont("helvetica", "bold");
  doc.setFontSize(10);
  doc.text("Subject: Notice of Security Agreement and Grant of Security Interest", margin, y);
  y += 7;
  ...
  // BODY
  para(`I am writing to formally confirm the Security Agreement entered into as of ${values.effectiveDate || "N/A"}, between ${values.party1Name || "the Secured Party"} (${values.party1Type === "business" ? "a business entity" : "an individual"}) and ${values.party2Name || "the Debtor"} (${values.party2Type === "business" ? "a business entity" : "an individual"}), governed by the laws of ${jurisdiction || "the applicable jurisdiction"}, effective immediately.`);

  para(values.description || "By this agreement, the Debtor grants to the Secured Party a continuing security interest in the collateral described herein to secure the payment and performance of all obligations owed. The Debtor represents that it has full authority to grant this security interest and that the collateral is free of any prior encumbrances except as disclosed.");

  para(`This agreement shall remain in effect for ${durationMap[values.duration] || values.duration || "the agreed duration"} and may be terminated upon ${terminationMap[values.terminationNotice] || values.terminationNotice || "the agreed notice"} written notice upon full satisfaction of all secured obligations. Disputes shall be resolved by ${disputeMap[values.disputeResolution] || values.disputeResolution || "the agreed method"}.${values.confidentiality === "yes" ? " A confidentiality clause is included and binding on both parties." : ""}${values.paymentAmount ? ` The secured amount is ${values.paymentAmount} payable on a ${values.paymentSchedule || "mutually agreed"} basis.` : ""}`);
  ...
  doc.setFont("helvetica", "bold");
  doc.setFontSize(10);
  const senderName = values.party1Name || "Secured Party";
  ...
  doc.setFont("helvetica", "bold");
  doc.setFontSize(10);
  doc.text("Secured Party Signature:", margin, y);
  doc.setFont("helvetica", "normal");
  doc.text(values.party1Signature || "________________________", margin + doc.getTextWidth("Secured Party Signature:  "), y);
  y += 7;
  doc.setFont("helvetica", "bold");
