...
const generatePDF = (values: Record<string, string>) => {
  ...
  doc.setFont("helvetica", "bold");
  doc.setFontSize(10);
  doc.text("Subject: Notice of Domestic Service Agreement and Terms of Engagement", margin, y);
  y += 7;
  ...
  // BODY
  para(`I am writing to formally confirm the Domestic Service Agreement entered into as of ${values.effectiveDate || "N/A"}, between ${values.party1Name || "the Client"} (${values.party1Type === "business" ? "a business entity" : "an individual"}) and ${values.party2Name || "the Service Provider"} (${values.party2Type === "business" ? "a business entity" : "an individual"}), governed by the laws of ${jurisdiction || "the applicable jurisdiction"}, effective immediately.`);

  para(values.description || "The Service Provider agrees to perform domestic services at the Client's premises as mutually agreed upon. The Service Provider shall carry out all duties with due care and diligence, and the Client agrees to provide a safe working environment and all necessary materials required to perform the services.");
  ...
  doc.setFont("helvetica", "bold");
  doc.setFontSize(10);
  const senderName = values.party1Name || "Client";
  ...
  doc.setFont("helvetica", "normal");
  doc.text(values.party1Signature || "________________________", margin + doc.getTextWidth("Client Signature:  "), y);
  y += 7;
  doc.setFont("helvetica", "bold");
  doc.text("Service Provider Signature:", margin, y);
  doc.setFont("helvetica", "normal");
  doc.text(values.party2Signature || "________________________", margin + doc.getTextWidth("Service Provider Signature:  "), y);
  y += 7;
