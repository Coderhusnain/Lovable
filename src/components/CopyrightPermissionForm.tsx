...
const generatePDF = (values: Record<string, string>) => {
  ...
  doc.setFontSize(13);
  const title = "COPYRIGHT PERMISSION";
  ...
  doc.setFont("helvetica", "bold");
  doc.setFontSize(10);
  doc.text("Subject: Notice of Copyright Permission", margin, y);
  y += 7;
  ...
  underlined("Rights Holder:  ", values.party1Name || "N/A");
  underlined("Rights Holder Address:  ", party1Address || "N/A");
  underlined("Rights Holder Email:  ", values.party1Email || "N/A");
  underlined("Requesting Party:  ", values.party2Name || "N/A");
  underlined("Requesting Party Address:  ", party2Address || "N/A");
  underlined("Requesting Party Email:  ", values.party2Email || "N/A");
  y += 2;

  para(`I am writing to formally confirm the Copyright Permission entered into as of ${values.effectiveDate || "N/A"}, between ${values.party1Name || "the Rights Holder"} and ${values.party2Name || "the Requesting Party"}, governed by the laws of ${jurisdiction || "the applicable jurisdiction"}.`);

  para(`${values.description || "The Rights Holder hereby grants permission to the Requesting Party to use the copyrighted material(s) specified herein for the purpose(s) described, subject to the conditions outlined in this document. This permission is limited to the specific use authorized and does not constitute a general license or transfer of rights."}`);

  para(`This permission shall remain in effect for ${durationMap[values.duration] || values.duration || "the agreed duration"} and may be revoked upon ${terminationMap[values.terminationNotice] || values.terminationNotice || "the agreed notice"} written notice. Disputes shall be resolved by ${disputeMap[values.disputeResolution] || values.disputeResolution || "the agreed method"}.${values.confidentiality === "yes" ? " A confidentiality clause is included." : ""}${values.paymentAmount ? ` Consideration for this permission is ${values.paymentAmount} payable on a ${scheduleMap[values.paymentSchedule] || values.paymentSchedule || "mutually agreed"} basis.` : ""}`);

  if (values.additionalTerms?.trim()) para(values.additionalTerms.trim());

  para(`Please retain a signed copy of this permission for your records. Both parties are bound by the terms stated herein from the effective date.`);
  ...
  doc.setFont("helvetica", "bold");
  doc.setFontSize(10);
  const senderName = values.party1Name || "Rights Holder";
  ...
  doc.setFont("helvetica", "normal");
  doc.text(values.party1Signature || "________________________", margin + doc.getTextWidth("Rights Holder Signature:  "), y);
  y += 7;
  doc.setFont("helvetica", "bold");
  doc.text("Requesting Party Signature:", margin, y);
  doc.setFont("helvetica", "normal");
  doc.text(values.party2Signature || "________________________", margin + doc.getTextWidth("Requesting Party Signature:  "), y);
  y += 7;
  ...
  doc.save("copyright_permission.pdf");
};

export default function CopyrightPermission() {
  return (
    <FormWizard
      steps={steps}
      ...
      documentType="copyrightpermission"
    />
  );
}