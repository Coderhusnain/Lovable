...
const generatePDF = (values: Record<string, string>) => {
  ...
  // Helper: underlined header field
  ...
  // Helper: inline underlined credential
  ...
  doc.setFont("helvetica", "bold");
  doc.setFontSize(10);
  doc.text("Subject: Notice of Accounting Contract and Terms of Engagement", margin, y);
  y += 7;
  ...
  // CREDENTIALS — underlined
  underlined("Accountant/Firm:  ", values.party1Name || "N/A");
  underlined("Client:  ", values.party2Name || "N/A");
  underlined("Client Address:  ", party2Address || "N/A");
  underlined("Client Email:  ", values.party2Email || "N/A");
  if (values.party2Phone) underlined("Client Phone:  ", values.party2Phone);
  y += 2;

  // BODY
  para(`I am writing to formally confirm the Accounting Contract entered into as of ${values.effectiveDate || "N/A"}, between ${values.party1Name || "the Accountant"} (${values.party1Type === "business" ? "a business entity" : "an individual"}) and ${values.party2Name || "the Client"} (${values.party2Type === "business" ? "a business entity" : "an individual"}), governed by the laws of ${jurisdiction || "the applicable jurisdiction"}.`);

  para(values.description || "The Accountant agrees to provide professional accounting, bookkeeping, financial reporting, and advisory services as required by the Client. All services shall be performed in accordance with applicable professional standards and regulations, and the Accountant shall maintain the confidentiality of all financial information provided.");

  para(`This contract shall remain in effect for ${durationMap[values.duration] || values.duration || "the agreed duration"} and may be terminated upon ${terminationMap[values.terminationNotice] || values.terminationNotice || "the agreed notice"} written notice. Disputes shall be resolved by ${disputeMap[values.disputeResolution] || values.disputeResolution || "the agreed method"}.${values.confidentiality === "yes" ? " A confidentiality clause is included and binding on both parties." : ""}${values.paymentAmount ? ` The agreed fee is ${values.paymentAmount} payable on a ${scheduleMap[values.paymentSchedule] || values.paymentSchedule || "mutually agreed"} basis.` : ""}`);

  if (values.additionalTerms?.trim()) para(values.additionalTerms.trim());

  para("Please retain a signed copy of this contract for your records. Both parties are bound by the terms stated herein from the effective date.");
  ...
  doc.setFont("helvetica", "bold");
  doc.setFontSize(10);
  const senderName = values.party1Name || "Accountant";
  ...
  doc.setFont("helvetica", "bold");
  doc.setFontSize(10);
  doc.text("Accountant Signature:", margin, y);
  doc.setFont("helvetica", "normal");
  doc.text(values.party1Signature || "________________________", margin + doc.getTextWidth("Accountant Signature:  "), y);
  y += 7;
  doc.setFont("helvetica", "bold");
  doc.text("Client Signature:", margin, y);
  doc.setFont("helvetica", "normal");
  doc.text(values.party2Signature || "________________________", margin + doc.getTextWidth("Client Signature:  "), y);
  y += 7;
...
export default function AccountingContract() {
  return (
    <FormWizard
      steps={steps}
