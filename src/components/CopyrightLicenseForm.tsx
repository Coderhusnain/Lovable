...
const generatePDF = (values: Record<string, string>) => {
  ...
  doc.setFont("helvetica", "bold");
  doc.setFontSize(10);
  doc.text("Subject: Notice of Copyright License Agreement", margin, y);
  y += 7;
  ...
  underlined("Licensor (Copyright Owner):  ", values.party1Name || "N/A");
  underlined("Licensor Address:  ", party1Address || "N/A");
  underlined("Licensor Email:  ", values.party1Email || "N/A");
  underlined("Licensee:  ", values.party2Name || "N/A");
  underlined("Licensee Address:  ", party2Address || "N/A");
  underlined("Licensee Email:  ", values.party2Email || "N/A");
  y += 2;

  para(`I am writing to formally confirm the Copyright License Agreement entered into as of ${values.effectiveDate || "N/A"}, between ${values.party1Name || "the Licensor"} and ${values.party2Name || "the Licensee"}, governed by the laws of ${jurisdiction || "the applicable jurisdiction"}.`);

  para(`${values.description || "The Licensor hereby grants to the Licensee a non-exclusive, non-transferable license to use the copyrighted work(s) described herein, subject to the terms and conditions of this agreement. The Licensee agrees to use the licensed work only as permitted herein and shall not sublicense, assign, or transfer any rights without prior written consent from the Licensor."}`);

  para(`This license shall remain in effect for ${durationMap[values.duration] || values.duration || "the agreed duration"} and may be terminated upon ${terminationMap[values.terminationNotice] || values.terminationNotice || "the agreed notice"} written notice. Disputes shall be resolved by ${disputeMap[values.disputeResolution] || values.disputeResolution || "the agreed method"}.${values.confidentiality === "yes" ? " A confidentiality clause is included." : ""}${values.paymentAmount ? ` The license fee is ${values.paymentAmount} payable on a ${scheduleMap[values.paymentSchedule] || values.paymentSchedule || "mutually agreed"} basis.` : ""}`);

  if (values.additionalTerms?.trim()) para(values.additionalTerms.trim());

  para(`Please retain a signed copy of this license for your records. Both parties are bound by the terms stated herein from the effective date.`);
  ...
  doc.save("copyright_license.pdf");
};
