...
// Helper: draw underlined text, returns text width
const underlineText = (doc: jsPDF, text: string, x: number, y: number, fontSize: number = 10) => {
  doc.text(text, x, y);
...
const labelWithUnderline = (doc: jsPDF, label: string, value: string, x: number, y: number) => {
  ...
  underlineText(doc, value, valueX, y);
};
...
const generatePDF = (values: Record<string, string>) => {
  ...
  doc.text("Manufacturing License", 105, y, { align: "center" });
  // Underline title
  const titleWidth = doc.getTextWidth("Manufacturing License");
  ...
  if (values.party1Phone) {
    labelWithUnderline(doc, "   Phone:  ", values.party1Phone, 20 + doc.getTextWidth("Email:  " + (values.party1Email || "")), y);
  }
  y += 10;
  ...
  if (values.party2Phone) {
    labelWithUnderline(doc, "   Phone:  ", values.party2Phone, 20 + doc.getTextWidth("Email:  " + (values.party2Email || "")), y);
  }
  y += 15;
  ...
  doc.save("manufacturing_license.pdf");
};
