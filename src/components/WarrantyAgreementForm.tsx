...
export default function WarrantyAgreementForm() {
  ...
  // Split fields into steps (max 3 per step, grouped by logical section)
  const steps = [
    {
      ...
      content: (
        <>
          ...
          <Input type="date" name="effectiveDate" value={formData.effectiveDate} onChange={e => setFormData(f => ({ ...f, effectiveDate: e.target.value }))} />
          <Label className="pt-2">Manufacturer - Name</Label>
          <Input name="manufacturerName" value={formData.manufacturerName} onChange={e => setFormData(f => ({ ...f, manufacturerName: e.target.value }))} />
          <Label>Manufacturer - Address</Label>
          <Textarea name="manufacturerAddress" value={formData.manufacturerAddress} onChange={e => setFormData(f => ({ ...f, manufacturerAddress: e.target.value }))} />
          <Label className="pt-2">Customer - Name</Label>
          <Input name="customerName" value={formData.customerName} onChange={e => setFormData(f => ({ ...f, customerName: e.target.value }))} />
          <Label>Customer - Address</Label>
          <Textarea name="customerAddress" value={formData.customerAddress} onChange={e => setFormData(f => ({ ...f, customerAddress: e.target.value }))} />
          <Label className="pt-2">Product Name</Label>
          <Input name="productName" value={formData.productName} onChange={e => setFormData(f => ({ ...f, productName: e.target.value }))} />
          <Label>Product Type</Label>
          <Input name="productType" value={formData.productType} onChange={e => setFormData(f => ({ ...f, productType: e.target.value }))} />
          <Label>Model Number</Label>
          <Input name="modelNumber" value={formData.modelNumber} onChange={e => setFormData(f => ({ ...f, modelNumber: e.target.value }))} />
          <Label>Serial Number</Label>
          <Input name="serialNumber" value={formData.serialNumber} onChange={e => setFormData(f => ({ ...f, serialNumber: e.target.value }))} />
          <Label className="pt-2">Warranty Period</Label>
          <Input name="warrantyPeriod" value={formData.warrantyPeriod} onChange={e => setFormData(f => ({ ...f, warrantyPeriod: e.target.value }))} />
          <Label>Warranty Commencement Date</Label>
          <Input type="date" name="warrantyCommenceDate" value={formData.warrantyCommenceDate} onChange={e => setFormData(f => ({ ...f, warrantyCommenceDate: e.target.value }))} />
        </>
      ),
      validate: () => !!formData.effectiveDate && !!formData.manufacturerName && !!formData.customerName,
    },
    {
      label: "Scope, Exclusions & Void Conditions",
      content: (
        <>
          <Label>Scope of Warranty (override)</Label>
          <Textarea name="scopeDetails" value={formData.scopeDetails} onChange={e => setFormData(f => ({ ...f, scopeDetails: e.target.value }))} />
          <Label className="pt-2">Exclusions</Label>
          <Textarea name="exclusions" value={formData.exclusions} onChange={e => setFormData(f => ({ ...f, exclusions: e.target.value }))} />
          <Label className="pt-2">Conditions Rendering Warranty Void</Label>
          <Textarea name="voidConditions" value={formData.voidConditions} onChange={e => setFormData(f => ({ ...f, voidConditions: e.target.value }))} />
        </>
      ),
    },
    {
      label: "Obtaining Service & Remedies",
      content: (
        <>
          <Label>Customer Obligations</Label>
          <Textarea name="customerObligations" value={formData.customerObligations} onChange={e => setFormData(f => ({ ...f, customerObligations: e.target.value }))} />
          <Label className="pt-2">In-Home Service Contact (toll-free)</Label>
          <Input name="inHomeContact" value={formData.inHomeContact} onChange={e => setFormData(f => ({ ...f, inHomeContact: e.target.value }))} />
          <Label>In-Home Service - Response Days</Label>
          <Input name="inHomeDays" value={formData.inHomeDays} onChange={e => setFormData(f => ({ ...f, inHomeDays: e.target.value }))} />
          <Label className="pt-2">Mail Return Instructions</Label>
          <Textarea name="mailReturnInstructions" value={formData.mailReturnInstructions} onChange={e => setFormData(f => ({ ...f, mailReturnInstructions: e.target.value }))} />
          <Label className="pt-2">Buyer Remedies (override)</Label>
          <Textarea name="buyerRemedies" value={formData.buyerRemedies} onChange={e => setFormData(f => ({ ...f, buyerRemedies: e.target.value }))} />
        </>
      ),
    },
    {
      label: "Legal, Governing Law & Signatures",
      content: (
        <>
          <Label>ADR / Dispute Resolution Notes</Label>
          <Textarea name="adrNotes" value={formData.adrNotes} onChange={e => setFormData(f => ({ ...f, adrNotes: e.target.value }))} />
          <Label className="pt-2">Severability (override)</Label>
          <Textarea name="severabilityNotes" value={formData.severabilityNotes} onChange={e => setFormData(f => ({ ...f, severabilityNotes: e.target.value }))} />
          <Label className="pt-2">Limitation of Liability (override)</Label>
          <Textarea name="limitationNotes" value={formData.limitationNotes} onChange={e => setFormData(f => ({ ...f, limitationNotes: e.target.value }))} />
          <Label className="pt-2">Governing Law</Label>
          <Input name="governingLaw" value={formData.governingLaw} onChange={e => setFormData(f => ({ ...f, governingLaw: e.target.value }))} />
          <hr />
          <Label>Manufacturer Signatory - Name</Label>
          <Input name="manufacturerSignName" value={formData.manufacturerSignName} onChange={e => setFormData(f => ({ ...f, manufacturerSignName: e.target.value }))} />
          <Label>Manufacturer Signatory - Date</Label>
          <Input type="date" name="manufacturerSignDate" value={formData.manufacturerSignDate} onChange={e => setFormData(f => ({ ...f, manufacturerSignDate: e.target.value }))} />
          <hr />
          <Label>Customer Signatory - Name</Label>
          <Input name="customerSignName" value={formData.customerSignName} onChange={e => setFormData(f => ({ ...f, customerSignName: e.target.value }))} />
          <Label>Customer Signatory - Date</Label>
          <Input type="date" name="customerSignDate" value={formData.customerSignDate} onChange={e => setFormData(f => ({ ...f, customerSignDate: e.target.value }))} />
        </>
      ),
    },
  ];

  // PDF generation logic for onFinish
  const handleFinish = () => {
    ...
    // ...existing PDF generation logic (copy from above)
  ...
  return <FormWizard steps={steps} onFinish={handleFinish} />;
}
