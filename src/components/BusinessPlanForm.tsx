...
const generatePDF = (values: Record<string, string>) => {
  ...
  doc.setFontSize(13);
  const title = "BUSINESS PLAN AGREEMENT";
  ...
  doc.setFont("helvetica", "bold");
  doc.setFontSize(10);
  doc.text("Subject: Notice of Business Plan Agreement and Terms of Engagement", margin, y);
  y += 7;
  ...
  // BODY
  para(`I am writing to formally confirm the Business Plan Agreement entered into as of ${values.effectiveDate || "N/A"}, between ${values.party1Name || "the First Party"} (${values.party1Type === "business" ? "a business entity" : "an individual"}) and ${values.party2Name || "the Second Party"} (${values.party2Type === "business" ? "a business entity" : "an individual"}), governed by the laws of ${jurisdiction || "the applicable jurisdiction"}, effective immediately.`);

  para(values.description || "Both parties agree to the terms of this business plan, which outlines the strategic objectives, operational framework, financial projections, and responsibilities of each party in advancing the agreed business initiative in accordance with applicable laws and regulations.");
