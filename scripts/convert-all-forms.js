...
// Template for granular form conversion
function generateGranularForm(formName, documentType, title, subtitle) {
  const cleanName = formName.replace('Form.tsx', '').replace('.tsx', '');
  const displayTitle = title || cleanName.replace(/([A-Z])/g, ' $1').trim();
...
const generatePDF = (values: Record<string, string>) => {
  ...
  doc.save("${cleanName.replace(/([A-Z])/g, '_$1').toLowerCase()}.pdf");
};
...
export default function ${cleanName}() {
  return (
    <FormWizard
      steps={steps}
      title="${displayTitle}"
      subtitle="${subtitle || 'Complete this form step by step'}"
      onGenerate={generatePDF}
      documentType="${documentType || cleanName.toLowerCase()}"
    />
  );
}
`;
...
const formFiles = fs.readdirSync(COMPONENTS_DIR).filter(f => 
  (f.endsWith('Form.tsx') || f.includes('Agreement')) && 
  !f.includes('FormWizard') &&
  !f.includes('UserInfoStep') &&
  !f.includes('CommunityFeed') &&
  !f.includes('PostCard') &&
  !f.includes('CreatePostModal') &&
  !f.includes('DocumentAboutSidebar') &&
  !f.includes('DocumentInfoLanding') &&
  !f.includes('LegalConcernsSection') &&
  !f.includes('LegalDisclaimer')
);

console.log(\`Found \${formFiles.length} form files to convert\`);
...
formFiles.forEach(file => {
  try {
    ...
    // Skip if already converted (has granular steps pattern)
    ...
    const formName = file;
    const cleanName = file.replace('Form.tsx', '').replace('.tsx', '');
    const displayTitle = cleanName.replace(/([A-Z])/g, ' $1').trim();
    
    const newContent = generateGranularForm(formName, cleanName.toLowerCase(), displayTitle, 'Complete each step to generate your document');
